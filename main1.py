...
if __name__ == '__main__':
    ...
    if listener == JAPANESE:
        ...
        # unpack statistics
        ckpt, accs, losses, models, ckpts = stats
        ...
        model, i, stats = retrain(models,
                                  JE_train_X, JE_train_y,
                                  test_X=JE_test_X,
                                  test_y=JE_test_y,
                                  num_epochs=args.num_epochs,
                                  num_iterations=args.num_iterations)

        save(model, i, stats, filename='weights/jl2_model_l%d_d%d' % (args.num_layers, args.num_dim))
        ...
        plot_acc_loss(accs, losses, i,
                      listener=listener,
                      filename="data/plots/jl2_en_acc_loss_l%d_d%d.png" % (args.num_layers, args.num_dim))

        # plot testing data
        labels = [u'la', u'li', u'lu', u'le', u'lo']
        plot_activations(ckpt, labels,
                         listener=listener,
                         filename="data/plots/jl2_el_act_l%d_d%d.png" % (args.num_layers, args.num_dim))

        labels = [u'ra', u'ri', u'ru', u're', u'ro']
        plot_activations(ckpt, labels,
                         listener=listener,
                         filename="data/plots/jl2_er_act_l%d_d%d.png" % (args.num_layers, args.num_dim))

        labels = [u'ら', u'り', u'る', u'れ', u'ろ']
        plot_activations(ckpt, labels,
                         listener=listener,
                         filename="data/plots/jl2_jp_act_l%d_d%d.png" % (args.num_layers, args.num_dim))
